...
func TestSupabaseDumpArgs(t *testing.T) {
	...
	prog, args, env, err := supabaseDumpArgs(cred, "postgres", schemas)
	if err != nil {
	...
	// Single pass: schema + data, so NEITHER --schema-only nor --data-only.
	if strings.Contains(joined, "--schema-only") || strings.Contains(joined, "--data-only") {
		t.Errorf("single dump must not split schema/data: %v", args)
	}
	for _, s := range []string{"--schema=public", "--schema=auth", "--schema=storage", "--schema=app"} {
		if !strings.Contains(joined, s) {
			t.Errorf("dump missing %s: %v", s, args)
		}
	}
	if strings.Contains(joined, "--schema=vault") || strings.Contains(joined, "--schema=realtime") {
		t.Errorf("dump must never include a managed schema: %v", args)
	}
	if !strings.Contains(joined, "-Fc") {
		t.Errorf("dump must use custom format -Fc: %v", args)
	}
	...
	if !contains(env, "PGCONNECT_TIMEOUT=15") {
		t.Errorf("env must set a connect timeout, got %v", env)
	}
}

func TestSupabaseDumpArgsValidation(t *testing.T) {
	cred := &CredEntry{Username: "postgres"}
	if _, _, _, err := supabaseDumpArgs(nil, "db", []string{"public"}); err == nil {
		t.Error("nil cred should error")
	}
	if _, _, _, err := supabaseDumpArgs(cred, "", []string{"public"}); err == nil {
		t.Error("empty db should error")
	}
	if _, _, _, err := supabaseDumpArgs(cred, "db", nil); err == nil {
...
func TestSupabaseRestoreArgs(t *testing.T) {
	...
	prog, args, _, err := supabaseRestoreArgs(cred, "postgres", "/tmp/dump.dump")
	if err != nil {
	...
	j := strings.Join(args, " ")
	// Single restore: --disable-triggers (safe data load), no schema/data split.
	if !strings.Contains(j, "--disable-triggers") {
		t.Errorf("restore must use --disable-triggers: %v", args)
	}
	if strings.Contains(j, "--schema-only") || strings.Contains(j, "--data-only") {
		t.Errorf("single restore must not split schema/data: %v", args)
	}
	if strings.Contains(j, "--no-owner") {
		t.Errorf("must preserve ownership (no --no-owner) so RLS/grants stay correct: %v", args)
	}
	if !strings.Contains(j, "/tmp/dump.dump") {
		t.Errorf("artifact path missing: %v", args)
	}

	if _, _, _, err := supabaseRestoreArgs(cred, "postgres", ""); err == nil {
